...
uint32_t
max86150_init(max86150_context_t *ctx, nsx_i2c_config_t *i2c_config, uint16_t addr) {
...
max86150_get_register(const max86150_context_t *ctx, uint8_t reg, uint8_t mask) {
    ...
    uint32_t regVal;
    uint8_t value;
    nsx_i2c_write_read(ctx->i2c_config, ctx->addr, &regAddr, 1, &regVal, 1);
    value = regVal & 0x00FF;
    if (mask != 0xFF) {
        value &= mask;
    }
...
max86150_set_register(const max86150_context_t *ctx, uint8_t reg, uint8_t value, uint8_t mask) {
    ...
    i2cBuffer[0] = reg;
    i2cBuffer[1] = value;
    err = nsx_i2c_write(ctx->i2c_config, &(i2cBuffer[0]), 2, ctx->addr);
    return err;
}
...
max86150_read_fifo_samples(const max86150_context_t *ctx, uint32_t *buffer,
    ...
    nsx_i2c_write_read(ctx->i2c_config, ctx->addr, &regAddr, 1, temp, 3);
    ...
    nsx_i2c_write(ctx->i2c_config, &regAddr, 1, ctx->addr);
    bufferIdx = 0;
    for (size_t i = 0; i < numSamples; i++) {
        nsx_i2c_read(ctx->i2c_config, (void *)rdBytes, bytesPerSample, ctx->addr);
        rdBytesIdx = 0;
